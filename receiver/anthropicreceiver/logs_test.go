...
func TestEmitLogs_ClaudeCodeContextInOperationLog(t *testing.T) {
	tb, _, _, logsSink := newTestTelemetryBuilder(t)
	data := newTestRequestData()
	...
	// Check body has claude code context
	bodyMap := firstLog.Body().Map()
	...
	val, ok = bodyMap.Get("claude_code.project.path")
	require.True(t, ok, "should have project.path in operation log body")
	assert.Equal(t, "/home/user/my-project", val.Str())

	// Check attributes have claude code context
	attrs := firstLog.Attributes()
	val, ok = attrs.Get("claude_code.project.name")
	require.True(t, ok, "should have project.name in operation log attrs")
	assert.Equal(t, "my-project", val.Str())

	// Should not have session.id
	_, ok = bodyMap.Get("claude_code.session.id")
